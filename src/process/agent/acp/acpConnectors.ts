...
function isUsableNodeBinary(nodePath: string): boolean {
  try {
    ...
    return /^v(1[89]|[2-9]\d)\./.test(version);
  } catch {
    return false;
  }
}

function prependPathDirs(env: Record<string, string | undefined>, dirs: string[]): void {
  const separator = process.platform === 'win32' ? ';' : ':';
  const current = (env.PATH || '').split(separator).filter(Boolean);
  const next: string[] = [];
  const seen = new Set<string>();

  for (const dir of [...dirs, ...current]) {
...
function preferRealNodeOnPath(env: Record<string, string | undefined>): void {
  const isWindows = process.platform === 'win32';
  const nodeFile = isWindows ? 'node.exe' : 'node';
  const candidates = [findSuitableNodeBin(18, 17), '/usr/local/bin', '/opt/homebrew/bin', '/usr/bin'].filter(
    (dir): dir is string => Boolean(dir)
  );

  const usableNodeDirs = candidates.filter((dir) => {
    const nodePath = path.join(dir, nodeFile);
    return existsSync(nodePath) && isUsableNodeBinary(nodePath);
  });

  if (usableNodeDirs.length > 0) {
    prependPathDirs(env, usableNodeDirs);
  }
}

function isUsableNpxBinary(npxPath: string): boolean {
  try {
    const version = execFileSync(npxPath, ['--version'], {
      encoding: 'utf-8',
      timeout: 5000,
      stdio: ['pipe', 'pipe', 'pipe'],
    }).trim();
...
function resolveNativeNpxPath(env: Record<string, string | undefined>): string {
  const command = process.platform === 'win32' ? 'npx.cmd' : 'npx';
  const candidates = ['/usr/local/bin/npx', '/opt/homebrew/bin/npx', '/usr/bin/npx'];

  for (const candidate of candidates) {
    if (existsSync(candidate) && isUsableNpxBinary(candidate)) {
      return candidate;
    }
  }
  ...
  try {
    const resolved = execFileSync(process.platform === 'win32' ? 'where' : 'which', [command], {
      ...
      .map((line) => line.trim())
      .find(Boolean);
    if (resolved && isUsableNpxBinary(resolved)) {
      return resolved;
    }
  } catch {
    // fall through to the command name and let spawn report the actual error
  }

  return command;
}

async function prepareIsolatedCodexHome(env: Record<string, string | undefined>, workingDir: string): Promise<void> {
  const sourceHome = process.env.CODEX_HOME?.trim() || path.join(os.homedir(), '.codex');
  const targetHome = path.join(getPlatformServices().paths.getDataDir(), 'codex-acp-home');
  await fs.mkdir(targetHome, { recursive: true });

  const copyIfPresent = async (name: string): Promise<void> => {
    const source = path.join(sourceHome, name);
    const target = path.join(targetHome, name);
    try {
      await fs.cp(source, target, { recursive: true, force: true });
    } catch {
      // Auth layouts differ between Codex versions. Missing optional files are OK.
    }
  };

  await copyIfPresent('auth.json');
  await copyIfPresent('accounts');
  await copyIfPresent('installation_id');

  const escapedWorkingDir = JSON.stringify(workingDir);
  const config = [
    '# Generated by AionUi for embedded Codex ACP sessions.',
    '# Keep this isolated from ~/.codex/config.toml so global MCP/plugins/hooks do not block session/new.',
    '',
    '[features]',
    'js_repl = false',
    'apps = false',
    ...
    'codex_hooks = false',
    '',
    `[projects.${escapedWorkingDir}]`,
    'trust_level = "trusted"',
    '',
  ].join('\n');

  await fs.writeFile(path.join(targetHome, 'config.toml'), config, 'utf-8');
...
export async function prepareCleanEnv(): Promise<Record<string, string | undefined>> {
  ...
  // Keep bun available for `bun x`, but do not let bun's `node` shim shadow a
  // real Node.js binary inside ACP children. Codex ACP starts MCP/JS helpers
  // during session creation and hangs when `node` resolves to Bun's shim.
  preferRealNodeOnPath(merged);
...
export function spawnNpxBackend(
  ...
  _preferOffline: boolean,
  {
    ...
    extraArgs?: string[];
    detached?: boolean;
    runtime?: 'bundled-bun' | 'native-npx';
  } = {}
): SpawnResult {
  const useNativeNpx = runtime === 'native-npx';
  const spawnArgs = useNativeNpx
    ? ['-y', npxPackage, ...extraArgs]
  ...
  console.log(`[ACP-PERF] ${backend}: process spawned ${Date.now() - spawnStart}ms (${runtime})`);
...
async function prepareCodex(
  ...
  const cleanEnv = await prepareCleanEnv();
  await prepareIsolatedCodexHome(cleanEnv, workingDir);
  ...
  // Do not launch Codex ACP through `bun x`: bun injects a temporary `node`
  // shim into NODE/PATH, and Codex disables its JS/MCP runtime when that shim is
  // what `node` resolves to during `session/new`.
  return { cleanEnv, npxCommand: resolveNativeNpxPath(cleanEnv) };
}
...
async function connectNpxBackend(config: {
  ...
  const opts = {
    ...
    runtime: config.runtime ?? 'bundled-bun',
  };
    ...
    for (const [index, npxPackage] of codexPackageCandidates.entries()) {
      try {
        await connectNpxBackend({
          backend: 'codex',
          npxPackage,
          prepareFn: () => prepareCodex(npxPackage, workingDir),
          workingDir,
          ...hooks,
          runtime: 'native-npx',
        });
